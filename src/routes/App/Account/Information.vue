		...
		<v-col cols="12" md="7" lg="8">
			<v-card class="pa-8">
				...
				<v-form ref="generalForm" @submit="apply" lazy-validation v-model="form.general">
					<div class="my-4">
						<v-text-field :error-messages="formErrors.screen_name" v-model="user.screen_name" :label="$t('account.information.screenNameLabel')" class="mt-2" type="text"></v-text-field>
						<v-row>
							<v-col class="py-0">
								<v-text-field :error-messages="formErrors.first_name" v-model="user.first_name" :label="$t('account.information.firstNameLabel')" class="mt-2" type="text"></v-text-field>
							</v-col>
							<v-col class="py-0">
								<v-text-field :error-messages="formErrors.last_name" v-model="user.last_name" :label="$t('account.information.lastNameLabel')" class="mt-2" type="text"></v-text-field>
							</v-col>
						</v-row>
						<v-row>
							<v-col class="py-0">
								<v-select :error-messages="formErrors.gender" v-model="user.gender" :items="genders" :label="$t('account.information.genderLabel')" class="mt-2"></v-select>
							</v-col>
							<v-col class="py-0">
								<DirectUsSelect @mapped="results => m2m.language = results" :error-messages="formErrors.language" v-model="user.language" root-key="language_id" return-object item-text="english_title" item-value="id" multiple :items="languages" :label="$t('account.information.languagesLabel')" class="mt-2" />
							</v-col>
						</v-row>

						<v-text-field :error-messages="formErrors.quote" max v-model="user.quote" :label="$t('account.information.quoteLabel')" class="mt-2" type="text"></v-text-field>

						<HTMLEditorField :error-messages="formErrors.biography" :label="$t('account.information.biographyLabel')" class="mt-2" v-model="user.biography" />
					</div>
		...
		<v-col cols="12" md="5" lg="4">
			<v-card class="pa-8">
				...
				<v-form ref="passwordForm" @submit="resetPassword" lazy-validation v-model="form.password">
					...
					<v-btn type="submit" color="primary" style="width: 100%" :disabled="!isPasswordSectionDifferent" large>
