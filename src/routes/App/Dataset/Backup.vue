		...
		<v-data-table
			...
			<template v-slot:item="props">
				<tr v-bind:class="{ deleted: dataset.deletedRows.indexOf(props.item.index) !== -1 }">
					...
					<td class="shrink">
						<v-switch v-model="dataset.rows[props.item.index].is_active" color="success" hide-details class="ma-0 pa-0"></v-switch>
					</td>
					<td class="shrink">
						<span class="text-no-wrap">
							<template v-if="dataset.rows[props.item.index].created_at">
								{{dataset.rows[props.item.index].created_at | date}}
							</template>
...
export default Vue.extend({
	...
	methods: {
		...
		getItem(rowIdx, currentHeader) {
            ...
            const cell = this.dataset.rows[rowIdx].cells.find(entry => entry.dataset_column_id === header.id);
		...
		importCSV(skipDuplicate = this.modalImportCSV.skipDuplicate) {
            ...
            this.dataset.rows = this.dataset.rows.concat(items);
	        ...
	        if (this.modalEditData.multiple) {
	            ...
	            this.selected.forEach(item => {
                    this.modalEditData.data.forEach((data, index) => {
                        if (data !== '' && data !== null) {
                            this.dataset.rows[item.index][index + 1] = typeof data === 'string' ? data.trim() : data;
						}
					});
				});
	            ...
	            this.editableHeaders.forEach((header, index) => {
                    ...
                    row.cells.push(new DatasetCell(values));
				});
        ...
        manageColumns() {
            ...
            this.modalManageColumn.columns = [...this.dataset.columns];
        ...
        applyManageColumns() {
            ...
            this.modalManageColumn.columns.forEach((newColumn, newIdx) => {
                const oldColumnIdx = this.dataset.columns.findIndex(oldColumn => oldColumn.guid === newColumn.guid);
                if (oldColumnIdx === -1) {
            ...
            if (toMove.length > 0 || toAdd.length > 0) {
                const originalRows = [...this.dataset.rows];
                const modifiedRows = [...this.dataset.rows];
                ...
                if (toAdd.length > 0) {
                    ...
                    this.dataset.rows = [...modifiedRows];
				}

                if (toMove.length > 0) {
                    this.dataset.rows.forEach((row, currIdx) => {
                        const clonedRow = [...row];
                        toMove.forEach(([oldIdx, newIdx]) => {
                            const oldValue = originalRows[currIdx][oldIdx];
                            clonedRow[newIdx] = oldValue;
                        });
                        this.dataset.rows[currIdx] = [...clonedRow];
                    });
                }
            }

            this.dataset.columns = [...this.modalManageColumn.columns];
		...
		editItem (item) {
			this.editedIndex = this.dataset.rows.indexOf(item);
		...
		save () {
			if (this.editedIndex > -1) {
				Object.assign(this.dataset.rows[this.editedIndex], this.editedItem);
			} else {
				this.dataset.rows.push(this.editedItem);
			}
			this.close();
		},
	},
...
<style lang="scss" scoped>
	...
	.v-data-table {
		&::v-deep header {flex: 0;}
		&::v-deep .v-data-table__wrapper {flex: 1;}
		&::v-deep .v-data-footer {flex: 0;}

		&::v-deep tr.deleted td {
