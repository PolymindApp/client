<template>
    <Page :title="title" class="fill-height pa-0 d-flex flex-column w-100">

        <!-- ADD/EDIT MODAL -->
        <Modal
            v-model="modalEdit.visible"
            :title="modalEdit.index === -1 ? $t('admin.modal.add').toString() : $t('admin.modal.edit').toString()"
            max-width="800"
            scrollable
            persistent
        >
            <template #header_right>
                <!-- NAVIGATION -->
                <template v-if="modalEdit.index !== -1">
                    <v-divider v-if="$vuetify.breakpoint.smAndDown" class="my-3" />
                    <div class="d-flex align-center" style="gap: 1rem">
                        <v-btn :disabled="modalEdit.saving" icon @click="onEditDictionaryPrevious">
                            <v-icon>mdi-arrow-left</v-icon>
                        </v-btn>
                        <span class="text-center" style="flex: 1">
                            {{ modalEdit.index + 1 }} or {{ dictionaries.length }}
                        </span>
                        <v-btn :disabled="modalEdit.saving" icon @click="onEditDictionaryNext">
                            <v-icon>mdi-arrow-right</v-icon>
                        </v-btn>
                    </div>
                </template>
            </template>
            <template #body>
                <v-form ref="editDictionaryForm" v-model="modalEdit.isFormValid">

                    <!-- COVER -->
                    <MediaField
                        v-model="currentDictionary.data.cover"
                        :rules="[rules.required]"
                        class="w-100 mb-4"
                        clearable
                        hide-details="auto"
                    />

                    <!-- CATEGORY -->
                    <v-select
                        v-model="currentDictionary.data.dictionary_category_id"
                        :items="categories"
                        :label="$t('admin.dictionaries.category')"
                        :rules="[rules.required]"
                        class="mb-4"
                        item-value="data.id"
                        item-text="data.i18n[0].data.text"
                        outlined
                        clearable
                        hide-details="auto"
                    />

                    <!-- TITLE -->
                    <I18nField
                        v-model="currentDictionary.data.i18n"
                        :label="$t('admin.dictionaries.i18n.title')"
                        :languages="languages"
                        :rules="[rules.required]"
                        type="title"
                        outlined
                        clearable
                        hide-details="auto"
                    />

                    <!-- BODY -->
                    <I18nField
                        v-model="currentDictionary.data.i18n"
                        :label="$t('admin.dictionaries.i18n.body')"
                        :languages="languages"
                        :rules="[rules.required]"
                        type="body"
                        outlined
                        clearable
                        hide-details="auto"
                    />
                </v-form>
            </template>
            <template #left_actions>

            </template>
            <template #buttons>

                <!-- SAVE -->
                <v-btn :block="$vuetify.breakpoint.smAndDown" :loading="modalEdit.saving" :disabled="!canSaveDictionary" color="primary" large @click="onModalEditSave">
                    <span v-text="modalEdit.index === -1 ? $t('btn.create') : $t('btn.save')"></span>
                </v-btn>

                <!-- CANCEL -->
                <v-btn :block="$vuetify.breakpoint.smAndDown" :disabled="modalEdit.saving" outlined large @click="onModalEditClose">
                    <span v-text="$t('btn.cancel')"></span>
                </v-btn>
            </template>
        </Modal>

        <!-- ADD/EDIT ITEMS -->
        <Modal
            v-model="modalEditItems.visible"
            :title="modalEditItems.itemIndex === -1 ? $t('admin.modal.add').toString() : $t('admin.modal.edit').toString()"
            max-width="800"
            scrollable
            persistent
        >
            <template #header_right>

                <!-- NAVIGATION -->
                <template v-if="modalEditItems.itemIndex !== -1">
                    <v-divider v-if="$vuetify.breakpoint.smAndDown" class="my-3" />
                    <div class="d-flex align-center" style="gap: 1rem">
                        <v-btn :disabled="modalEditItems.saving" icon @click="onEditDictionaryItemsPrevious">
                            <v-icon>mdi-arrow-left</v-icon>
                        </v-btn>
                        <span class="text-center" style="flex: 1">
                            {{ modalEditItems.itemIndex + 1 }} or {{ modalEditItems.items.length }}
                        </span>
                        <v-btn :disabled="modalEditItems.saving" icon @click="onEditDictionaryItemsNext">
                            <v-icon>mdi-arrow-right</v-icon>
                        </v-btn>
                    </div>
                </template>
            </template>
            <template #body>
                <v-form ref="editDictionaryItemsForm" v-model="modalEditItems.isFormValid">

                    <!-- COVER -->
                    <MediaField
                        v-model="currentItem.data.cover"
                        :rules="[rules.required]"
                        hide-details="auto"
                        class="w-100 mb-4"
                        clearable
                    />

                    <!-- TITLE -->
                    <I18nField
                        v-model="currentItem.data.i18n"
                        :label="$t('admin.dictionaries.i18n.title')"
                        :languages="languages"
                        :rules="[rules.required]"
                        hide-details="auto"
                        outlined
                        clearable
                    >
                        <template #append="{ i18n }">
                            <PlayRecordCombo
                                v-model="i18n.data.text_synthesized"
                                :loading="isLoading"
                                :voices="voices"
                                :voice="i18n.data.voice_id"
                                class="d-flex align-center ml-3"
                            />
                        </template>
                    </I18nField>
                </v-form>
            </template>
            <template #left_actions>

                <!-- DELETE -->
                <v-btn :block="$vuetify.breakpoint.smAndDown" :loading="modalEditItems.saving" :disabled="!canDeleteDictionaryItem" text color="error" large @click="() => onDeleteDictionaryItemClick(modalEditItems.items[modalEditItems.itemIndex])">
                    <span v-text="$t('btn.delete')"></span>
                </v-btn>
            </template>
            <template #buttons>

                <!-- ADD -->
                <v-btn :block="$vuetify.breakpoint.smAndDown" :loading="modalEditItems.saving" :disabled="!canAddDictionaryItem" color="primary" large @click="onAddDictionaryItemClick">
                    <v-icon left>mdi-plus</v-icon>
                    <span v-text="$t('btn.add')"></span>
                </v-btn>

                <!-- SAVE -->
                <v-btn :block="$vuetify.breakpoint.smAndDown" :loading="modalEditItems.saving" :disabled="!canSaveDictionaryItems" color="primary" large @click="onModalEditItemsSave">
                    <span v-text="modalEditItems.itemIndex === -1 ? $t('btn.create') : $t('btn.save')"></span>
                </v-btn>

                <!-- CANCEL -->
                <v-btn :block="$vuetify.breakpoint.smAndDown" :disabled="modalEditItems.saving" outlined large @click="onModalEditItemsClose">
                    <span v-text="$t('btn.cancel')"></span>
                </v-btn>
            </template>
        </Modal>

        <!-- CONTENT -->
        <div class="pa-3 w-100" style="flex: 1">
            <v-card class="fill-height d-flex flex-column w-100">
                <v-card-title
                    class="d-flex align-center justify-space-between"
                    style="flex: 0; gap: 1rem"
                >
                    <!-- ADD NEW ITEM -->
                    <v-btn
                        :block="$vuetify.breakpoint.smAndDown"
                        :disabled="isLoading"
                        color="primary"
                        class="order-md-last"
                        @click="() => onEditDictionaryClick(-1)"
                    >
                        <v-icon left>mdi-plus-circle</v-icon>
                        <span v-text="$t('admin.dictionaries.addNew')"></span>
                    </v-btn>

                    <!-- SEARCH -->
                    <v-text-field
                        v-model="search"
                        :placeholder="$t('admin.dictionaries.filter')"
                        :disabled="isLoading"
                        outlined
                        dense
                        hide-details
                        prepend-inner-icon="mdi-magnify"
                        class="order-md-first"
                    >
                    </v-text-field>
                </v-card-title>
                <v-card-text
                    class="w-100 px-md-0 fill-height d-flex flex-column pb-0"
                    style="flex: 1"
                >
                    <!-- DATA (DESKTOP) -->
                    <v-data-table
                        :items="dictionaries"
                        :search="search"
                        :headers="headers"
                        :loading="loading"
                        :items-per-page.sync="itemPerPage"
                        :style="{
                            flexGrow: 1,
                            height: 0,
                        }"
                        fixed-header
                        item-key="data.id"
                        sort-by="data.created_at"
                        sort-desc
                        class="fill-height overflow-y-auto"
                        @update:page="index => page = index"
                    >
                        <!-- COVER -->
                        <template #item.data.cover.url="{ item }">
                            <v-img
                                :src="item.data.cover.data.url"
                                contain
                                height="64"
                                width="64"
                                class="align-center text-center"
                            >
                                <template #placeholder>
                                    <v-skeleton-loader type="image" />
                                </template>
                            </v-img>
                        </template>

                        <!-- LANGUAGES -->
                        <template #item.data.languages="{ item }">
                            <span v-if="item.data.i18n.length === 0" v-text="$t('admin.dictionaries.noLanguage')"></span>
                            <div v-else class="d-flex align-center flex-wrap" style="gap: 0.5rem">
                                <v-chip
                                    v-for="language in item.languages().splice(0, 3)"
                                    :key="language.data.language.data.code"
                                    x-small
                                >
                                    <span v-text="language.data.language.data.name"></span>
                                </v-chip>
                                <v-chip
                                    v-if="item.languages().length > 3" v-text="'+ ' + (item.languages().length - 3)"
                                    x-small
                                    color="opacity-33"
                                ></v-chip>
                            </div>
                        </template>

                        <!-- CATEGORY -->
                        <template #item.data.category="{ item }">
                            <span v-if="!item.data.category.data.id" v-text="$t('admin.dictionaries.noCategory')" class="text-no-wrap"></span>
                            <span v-else v-text="item.data.category.data.i18n[0].data.text"></span>
                        </template>

                        <!-- TOTAL ITEMS -->
                        <template #item.data.total_items="{ item }">
                            <v-chip v-text="item.data.total_items" small></v-chip>
                        </template>

                        <!-- CREATED AT -->
                        <template #item.data.created_at="{ item }">
                            <span class="text-no-wrap">{{ item.data.created_at | humanDate }}</span>
                        </template>

                        <!-- ACTIONS -->
                        <template #item._action="{ item, index }">
                            <div class="d-flex">

                                <!-- EDIT -->
                                <v-tooltip left>
                                    <template #activator="{ attrs, on }">
                                        <v-btn v-bind="attrs" v-on="on" :disabled="isLoading" icon @click.stop="onEditDictionaryClick(item)">
                                            <v-icon>mdi-pencil</v-icon>
                                        </v-btn>
                                    </template>
                                    <span v-text="$t('btn.edit')"></span>
                                </v-tooltip>

                                <!-- EDIT ITEMS -->
                                <v-tooltip left>
                                    <template #activator="{ attrs, on }">
                                        <v-btn v-bind="attrs" v-on="on" :disabled="isLoading" icon :loading="modalEditItems.loadingIndex === index" @click="onEditDictionaryItemsClick(item)">
                                            <v-icon>mdi-cards</v-icon>
                                        </v-btn>
                                    </template>
                                    <span v-text="$t('btn.editItems')"></span>
                                </v-tooltip>

                                <!-- DELETE -->
                                <v-tooltip left>
                                    <template #activator="{ attrs, on }">
                                        <v-btn v-bind="attrs" v-on="on" :disabled="isLoading" icon @click.stop="onDeleteDictionaryClick(item)">
                                            <v-icon>mdi-delete</v-icon>
                                        </v-btn>
                                    </template>
                                    <span v-text="$t('btn.delete')"></span>
                                </v-tooltip>
                            </div>
                        </template>
                    </v-data-table>
                </v-card-text>
            </v-card>
        </div>
    </Page>
</template>

<script lang="ts">
import Page from "@/components/layout/Page.vue";
import Modal from "@/components/generic/Modal.vue";
import DictionaryModel from '@/models/DictionaryModel';
import Services from "@/utils/Services";
import MediaField from "@/components/fields/MediaField.vue";
import I18nField from "@/components/fields/I18nField.vue";
import PlayRecordCombo from "@/components/audio/PlayRecordCombo.vue";
import LanguageModel from "@/models/LanguageModel";
import DictionaryCategoryModel from "@/models/DictionaryCategoryModel";
import VueI18n from "vue-i18n";
import Rules from "@/utils/Rules";
import DictionaryItemModel from "@/models/DictionaryItemModel";
import { Component, Vue, Watch, Ref } from 'vue-property-decorator';
import VoiceModel from "@/models/VoiceModel";

@Component({
    components: {
        MediaField,
        I18nField,
        PlayRecordCombo,
        Page,
        Modal,
    }
})
export default class Dictionaries extends Vue {

    @Ref() readonly editDictionaryForm: any;
    @Ref() readonly editDictionaryItemsForm: any;

    title: string | VueI18n.TranslateResult
    search: string = ''
    loading: boolean = false
    skeleton: boolean = false
    page = 1
    itemPerPage = 10
    dictionaries: Array<DictionaryModel> = []
    voices: Array<VoiceModel> = []
    languages: Array<LanguageModel> = []
    categories: Array<DictionaryCategoryModel> = []
    rules: any = {}
    currentDictionary = new DictionaryModel();
    currentItem = new DictionaryItemModel();
    modalEdit: {
        visible: boolean,
        isFormValid: boolean,
        index: number,
        saving: boolean,
    } = {
        visible: false,
        isFormValid: false,
        index: -1,
        saving: false,
    }
    modalEditItems: {
        visible: boolean,
        isFormValid: false,
        dictionaryIndex: number,
        itemIndex: number,
        saving: boolean,
        loadingIndex: number,
        items: Array<DictionaryItemModel>,
    } = {
        visible: false,
        isFormValid: false,
        dictionaryIndex: -1,
        itemIndex: -1,
        saving: false,
        loadingIndex: -1,
        items: [],
    }

    @Watch('modalEdit.index')
    onModalEditIndexChanged(index: number) {
        if (index === -1) {
            this.currentDictionary = new DictionaryModel();
        } else {
            this.currentDictionary = this.dictionaries[index];
        }
    }

    @Watch('modalEditItems.itemIndex')
    onModalEditItemsIndexChanged(index: number) {
        if (index === -1) {
            this.currentItem = new DictionaryItemModel({
                dictionary_id: this.dictionaries[this.modalEditItems.dictionaryIndex].data.id,
            });
        } else {
            this.currentItem = this.modalEditItems.items[index];
        }
    }

    get canSaveDictionary(): boolean {
        return !this.modalEdit.saving && this.modalEdit.isFormValid;
    }

    get canSaveDictionaryItems(): boolean {
        return !this.modalEditItems.saving && this.modalEditItems.isFormValid;
    }

    get canDeleteDictionaryItem(): boolean {
        const dictionary = this.modalEditItems.items[this.modalEditItems.itemIndex];
        return !this.modalEditItems.saving && dictionary?.data.id;
    }

    get canAddDictionaryItem(): boolean {
        return !this.modalEditItems.saving;
    }

    get headers(): Array<any> {
        return [
            { value: 'data.cover.url', text: this.$i18n.t('admin.dictionaries.headers.cover'), class: 'text-no-wrap', width: 0, sortable: false },
            { value: 'data.i18n[0].data.text', text: this.$i18n.t('admin.dictionaries.headers.title'), class: 'text-no-wrap', },
            { value: 'data.languages', text: this.$i18n.t('admin.dictionaries.headers.languages'), class: 'text-no-wrap' },
            { value: 'data.category', text: this.$i18n.t('admin.dictionaries.headers.category'), class: 'text-no-wrap' },
            { value: 'data.total_items', text: this.$i18n.t('admin.dictionaries.headers.totalItems'), class: 'text-no-wrap', width: 0 },
            { value: 'data.created_at', text: this.$i18n.t('admin.dictionaries.headers.createdAt'), class: 'text-no-wrap' },
            { value: '_action', sortable: false },
        ];
    }

    get isLoading(): boolean {
        return this.loading ||
            this.modalEdit.saving ||
            (
                this.modalEditItems.loadingIndex !== this.modalEditItems.dictionaryIndex &&
                this.modalEditItems.loadingIndex !== -1
            );
    }

    get startPageIdx(): number {
        return (this.page - 1) * this.itemPerPage;
    }

    onEditDictionaryClick(item: DictionaryModel): void {
        const index = this.dictionaries.findIndex(dictionary => dictionary === item);
        if (index === this.modalEdit.index) {
            this.onModalEditIndexChanged(index);
        }

        Object.assign(this.modalEdit, {
            visible: true,
            index,
        });

        // Requires a timeout so the modal is created the first time
        setTimeout(() => {
            if (this.editDictionaryForm) {
                this.editDictionaryForm.resetValidation();
            }
        })
    }

    onEditDictionaryItemsClick(item: DictionaryModel): void {
        const index = this.dictionaries.findIndex(dictionary => dictionary === item);
        this.modalEditItems.loadingIndex = index;
        Services.getDictionaryItems(this.dictionaries[index].data.id)
            .then(items => {

                const itemIndex = items.length > 0 ? 0 : -1;

                Object.assign(this.modalEditItems, {
                    visible: true,
                    dictionaryIndex: index,
                    itemIndex,
                    items,
                });

                if (itemIndex === this.modalEditItems.itemIndex) {
                    this.onModalEditItemsIndexChanged(itemIndex);
                }

                // Requires a timeout so the modal is created the first time
                setTimeout(() => {
                    if (this.editDictionaryItemsForm) {
                        this.editDictionaryItemsForm.resetValidation();
                    }
                })
            })
            .catch(this.$handleError)
            .finally(() => this.modalEditItems.loadingIndex = -1);
    }

    onModalEditSave(): void {
        this.modalEdit.saving = true;
        Services.saveDictionary(this.currentDictionary)
            .then(dictionary => {
                if (this.modalEdit.index === -1) {
                    this.$snack(this.$i18n.t('admin.dictionaries.createdSnack'));
                    this.dictionaries.unshift(dictionary);
                } else {
                    this.$snack(this.$i18n.t('admin.dictionaries.updatedSnack'));
                }
                this.modalEdit.visible = false;
            })
            .catch(this.$handleError)
            .finally(() => this.modalEdit.saving = false);
    }

    onModalEditClose(): void {
        this.modalEdit.visible = false;
    }

    onModalEditItemsSave(): void {
        this.modalEditItems.saving = true;
        Services.saveDictionaryItems(this.currentItem)
            .then(dictionaryItem => {
                if (this.modalEditItems.itemIndex === -1) {
                    this.$snack(this.$i18n.t('admin.dictionaryItem.createdSnack'));
                    this.modalEditItems.items.unshift(dictionaryItem);
                } else {
                    this.$snack(this.$i18n.t('admin.dictionaryItem.updatedSnack'));
                }
                this.modalEditItems.visible = false;
            })
            .catch(this.$handleError)
            .finally(() => this.modalEditItems.saving = false);
    }

    onModalEditItemsClose(): void {
        this.modalEditItems.visible = false;
    }

    onEditDictionaryPrevious() {
        this.modalEdit.index--;
        if (this.modalEdit.index < 0) {
            this.modalEdit.index = this.dictionaries.length - 1;
        }
    }

    onEditDictionaryNext() {
        this.modalEdit.index++;
        if (this.modalEdit.index > this.dictionaries.length - 1) {
            this.modalEdit.index = 0;
        }
    }

    onEditDictionaryItemsPrevious() {
        this.modalEditItems.itemIndex--;
        if (this.modalEditItems.itemIndex < 0) {
            this.modalEditItems.itemIndex = this.modalEditItems.items.length - 1;
        }
    }

    onEditDictionaryItemsNext() {
        this.modalEditItems.itemIndex++;
        if (this.modalEditItems.itemIndex > this.modalEditItems.items.length - 1) {
            this.modalEditItems.itemIndex = 0;
        }
    }

    onDeleteDictionaryClick(item: DictionaryModel): void {
        const index = this.dictionaries.findIndex(dictionary => dictionary === item);
        this.$confirm(
            this.$i18n.t('admin.dictionaries.delete.title'),
            this.$i18n.t('admin.dictionaries.delete.desc'),
            this.$i18n.t('btn.delete'),
            (modal, btn) => {
                btn.attrs.loading = true;
                modal.disabled = true;

                Services.deleteDictionary(item.data.id)
                    .then(() => {
                        this.dictionaries.splice(index, 1);
                        this.$snack(this.$i18n.t('admin.dictionaries.deleteSnack'));
                        modal.visible = false;
                    })
                    .catch(this.$handleError)
                    .finally(() => this.loading = false);

            }, undefined, {
                color: 'error'
            }
        )
    }

    onDeleteDictionaryItemClick(item: DictionaryItemModel): void {
        const index = this.modalEditItems.items.findIndex(model => model === item);
        this.$confirm(
            this.$i18n.t('admin.dictionaries.deleteItem.title'),
            this.$i18n.t('admin.dictionaries.deleteItem.desc'),
            this.$i18n.t('btn.delete'),
            (modal, btn) => {
                btn.attrs.loading = true;
                modal.disabled = true;

                Services.deleteDictionaryItems(item.data.id)
                    .then(() => {
                        this.modalEditItems.items.splice(index, 1);
                        this.$snack(this.$i18n.t('admin.dictionaries.deleteItemSnack'));
                        modal.visible = false;
                    })
                    .catch(this.$handleError)
                    .finally(() => this.loading = false);

            }, undefined, {
                color: 'error'
            }
        )
    }

    onAddDictionaryItemClick(): void {
        const dictionary = this.dictionaries[this.modalEditItems.dictionaryIndex];
        this.modalEditItems.items.push(new DictionaryItemModel({
            dictionary_id: dictionary?.data.id,
        }))
        this.modalEditItems.itemIndex = this.modalEditItems.items.length - 1;
    }

    load(): Promise<Array<Array<any>>> {
        this.loading = true;
        return Promise.all([
            Services.getLanguages(),
            Services.getVoices(),
            Services.getDictionaries(),
            Services.getDictionaryCategories(),
        ])
            .then(([languages, voices, dictionaries, categories]) => {
                Object.assign(this, { languages, voices, dictionaries, categories });
                this.skeleton = false;
                return [languages, dictionaries];
            })
            .catch(this.$handleError)
            .finally(() => this.loading = false);
    }

    created(): void {
        this.title = this.$t('admin.dictionaries.title');
        this.rules.required = (value: string) => Rules.required(value) || this.$t('rules.required');
        this.load();
    }
}
</script>

<style lang="scss" scoped>
.v-data-table {
    display: flex;
    flex-direction: column;
    height: 100%;
}
.v-data-table ::v-deep .v-data-table__wrapper {
    display: flex;
    flex-direction: column;
    height: 100%;
    flex: 1;
}
.v-data-table ::v-deep .v-data-footer {
    flex: 0;
}
</style>
